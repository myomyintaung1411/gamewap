...
export default class WS {
  ...
  async startRunning(platform, url) {
    ...
    if (!url) return console.error('Error WsManager start: miss url');

    const langParam = getLangParam();

    if (this.PLATFORM === 'H5') {
      // browser: append lang as query param
      const hasQuery = url.includes('?');
      this.url = `${url}${hasQuery ? '&' : '?'}lang=${langParam}`;
    } else if (this.PLATFORM === 'APP-PLUS') {
      // native: keep URL, send lang in header
      this.url = url;
      this.wsHeader = { lang: langParam };
    }
