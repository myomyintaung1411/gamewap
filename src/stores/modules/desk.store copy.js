...
export const useDeskStore = defineStore({
	...
	actions: {
		...
		// 	}, delayGetList || 0)
		// },
		switchQueryList(force, queryType) {
			if (queryType === this.queryType) return;
      ...
      this.hasNext = false;
			...
			async initTypeList() {//获取本地类型
			console.time("room")
			return new Promise(resolve => {
				this.loading = true;
				// this.list.splice(0, this.list.length);
				console.log(this.allDesk,"选桌",this.queryType);
				if(this.queryType){
					let buff=[]
					buff = this.allDesk.filter((item)=>{
						if(item.deskTypeName==this.queryType){
							return true;
						}
					})
					this.list=[];
					this.list = buff.concat([])
				}else{
					this.list=[];
					this.list = this.allDesk.concat([])
				}
			...
			async initList() {
			console.log(this.allDesk,"获取服务器台桌列表同步=====>",this.queryMulti)
			window.$loading(true,"请稍等");
			console.time("room");
			return new Promise(resolve => {
				this.loading = true;
				this.hasNext = false;
				...
				multiStage: this.queryMulti,
					...
					this.allDesk = this.list.concat([]);
					console.log(this.allDesk,"<=====返回服务器台桌列表同步")
					!this.useDesk && (this.useDesk = this.list[0].deskId);
					resolve(true);
				})
			})
			...
			return new Promise(resolve => {
				...
				const _listenId = listenAdd('stageListNew  ', {
					// gameTypeId: '',
					// maxId: 0,
					pageNum: this.page,
				pageSize: _userStore().isPhoneBet ? 50 : this.pageSize,
